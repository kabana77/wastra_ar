...
uses DM, {MainBrowse, Main,} Menus{, Agen};

{$R *.dfm}

procedure TAgenFrm.FormClose(Sender: TObject;
...
procedure TAgenFrm.BtnSimpanClick(Sender: TObject);
begin
...
procedure TAgenFrm.QMasterAfterPost(DataSet: TDataSet);
begin
  BtnSimpan.Enabled:=True;
end;

procedure TAgenFrm.BtnEditingClick(Sender: TObject);
begin
...
procedure TAgenFrm.BtnBrowseClick(Sender: TObject);
begin
...
procedure TAgenFrm.BitBtn1Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
begin
...
procedure TAgenFrm.FormCreate(Sender: TObject);
begin
  //DMFrm.QSatuan.Close;
...
procedure TAgenFrm.wwDBGrid1TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
...
procedure TAgenFrm.QMasterAfterScroll(DataSet: TDataSet);
begin
  LRecords.Caption:='Data ke '+IntToStr(DataSet.RecNo)+' dari '+IntToStr(DataSet.RecordCount)+' data';
end;

procedure TAgenFrm.BitBtn5Click(Sender: TObject);
var
  vtitle : String;
begin
  case PageControl1.ActivePageIndex of
...
procedure TAgenFrm.TabSheet1Show(Sender: TObject);
begin
  QMaster.Close;
  QMaster.Open;
end;

procedure TAgenFrm.BtnEditing2Click(Sender: TObject);
begin
...
procedure TAgenFrm.BtnBrowse2Click(Sender: TObject);
begin
  if BtnSimpan.Enabled then
    ShowMessage('Simpan/ Posting Data Dulu !')
    else
...
procedure TAgenFrm.BitBtn3Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
begin
...
procedure TAgenFrm.wwDBGrid2TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
  if ((Sender as TwwDBGrid).ColumnByName(AFieldName).FieldName<>'') then
  begin
...
procedure TAgenFrm.TabSheet2Show(Sender: TObject);
begin
  QMaster2.Close;
  QMaster2.Open;
end;

procedure TAgenFrm.FormShow(Sender: TObject);
begin
  Height:=600;
  Width:=716;
  TabSheet2.TabVisible:=False;
...
procedure TAgenFrm.LookMesinEnter(Sender: TObject);
begin
  QMaster.Close;
  QMaster.Open;
end;

procedure TAgenFrm.LookJenisEnter(Sender: TObject);
begin
{  QMaster2.Close;
  QMaster2.Open; }
end;

procedure TAgenFrm.LookKonstruksiEnter(Sender: TObject);
begin
//  DMFrm.QKonstruksi.Open;
end;

procedure TAgenFrm.QMasterNewRecord(DataSet: TDataSet);
begin
...
procedure TAgenFrm.QMaster2NewRecord(DataSet: TDataSet);
begin
//  QMaster2KD_JNS_ITEM.AsString:='C';
end;

procedure TAgenFrm.FormCloseQuery(Sender: TObject;
...
procedure TAgenFrm.wwDBGrid1DblClick(Sender: TObject);
var
...
procedure TAgenFrm.wwDBGrid1CalcCellColors(Sender: TObject;
...
procedure TAgenFrm.LookSatuanEnter(Sender: TObject);
begin
  DMFrm.QSatuan.Open;
end;

procedure TAgenFrm.cbOtomatisClick(Sender: TObject);
begin
...
procedure TAgenFrm.ECariChange(Sender: TObject);
begin
  if cbOtomatis.Checked then
  begin
...
procedure TAgenFrm.LTitleClick(Sender: TObject);
var
  DC : HDC;
  ABitmap:TBitmap;
begin
...
procedure TAgenFrm.QMasterBeforeEdit(DataSet: TDataSet);
begin
// QMasterOPR_EDIT.AsString:=DMFrm.QUserNAMA_USER.AsString;
end;

procedure TAgenFrm.QMasterBeforePost(DataSet: TDataSet);
begin
// QMasterOPR_EDIT.AsString:=DMFrm.QUserNAMA_USER.AsString;
end;

procedure TAgenFrm.BitBtn6Click(Sender: TObject);
begin
{azmi
...
procedure TAgenFrm.BitBtn4Click(Sender: TObject);
begin
{azmi
if QLookUpKelKD_KEL.AsString='' then
begin
...
procedure TAgenFrm.wwDBLookupCombo1CloseUp(Sender: TObject; LookupTable,
  FillTable: TDataSet; modified: Boolean);
begin
{azmi
...
procedure TAgenFrm.wwDBLookupCombo1Enter(Sender: TObject);
begin
{azmi
QLookUpKel.Open;
azmi}
...
procedure TAgenFrm.BitBtn2Click(Sender: TObject);
{azmi}
var
  vuser, vpwd : string;
begin
...
procedure TAgenFrm.LookCountryClick(Sender: TObject);
begin
{  QMaster2.Close;
  QMaster2.Open; }
end;

procedure TAgenFrm.LookCountryEnter(Sender: TObject);
begin
  QMaster2.Close;
  QMaster2.Open;
end;
