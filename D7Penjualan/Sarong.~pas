...
var
  SarongFrm: TSarongFrm;

implementation

{uses DM, MainBrowse, Main, Menus;}
uses DM, Penjualan, Menus, Kemasan;

{$R *.dfm}

procedure TSarongFrm.FormClose(Sender: TObject;
...
procedure TSarongFrm.BtnSimpanClick(Sender: TObject);
begin
...
procedure TSarongFrm.QMasterAfterPost(DataSet: TDataSet);
begin
  BtnSimpan.Enabled:=True;
end;

procedure TSarongFrm.BtnEditingClick(Sender: TObject);
begin
...
procedure TSarongFrm.BtnBrowseClick(Sender: TObject);
begin
...
procedure TSarongFrm.BitBtn1Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
begin
...
procedure TSarongFrm.FormCreate(Sender: TObject);
begin
  //DMFrm.QSatuan.Close;
...
procedure TSarongFrm.wwDBGrid1TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
...
procedure TSarongFrm.QMasterAfterScroll(DataSet: TDataSet);
begin
  LRecords.Caption:='Data ke '+IntToStr(DataSet.RecNo)+' dari '+IntToStr(DataSet.RecordCount)+' data';
end;

procedure TSarongFrm.BitBtn5Click(Sender: TObject);
var
  vtitle : String;
begin
  {case PageControl1.ActivePageIndex of
...
procedure TSarongFrm.TabSheet1Show(Sender: TObject);
begin
  QMaster.Open;
end;

procedure TSarongFrm.BtnEditing2Click(Sender: TObject);
begin
...
procedure TSarongFrm.BtnBrowse2Click(Sender: TObject);
begin
  if BtnSimpan.Enabled then
    ShowMessage('Simpan/ Posting Data Dulu !')
    else
...
procedure TSarongFrm.BitBtn3Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
begin
...
procedure TSarongFrm.wwDBGrid2TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
  if ((Sender as TwwDBGrid).ColumnByName(AFieldName).FieldName<>'') then
  begin
...
procedure TSarongFrm.TabSheet2Show(Sender: TObject);
begin
  QMaster2.Open;
end;

procedure TSarongFrm.FormShow(Sender: TObject);
begin
  Height:=600;
  Width:=767;
end;

procedure TSarongFrm.LookMesinEnter(Sender: TObject);
begin
  QMaster.Open;
end;

procedure TSarongFrm.LookJenisEnter(Sender: TObject);
begin
  QMaster2.Open;
end;

procedure TSarongFrm.LookKonstruksiEnter(Sender: TObject);
begin
  //DMFrm.QKonstruksi.Open;
end;

procedure TSarongFrm.QMasterNewRecord(DataSet: TDataSet);
begin
...
procedure TSarongFrm.QMaster2NewRecord(DataSet: TDataSet);
begin
  QMaster2KD_JNS_ITEM.AsString:='5';
end;

procedure TSarongFrm.FormCloseQuery(Sender: TObject;
...
procedure TSarongFrm.wwDBGrid1DblClick(Sender: TObject);
var
...
procedure TSarongFrm.wwDBGrid1CalcCellColors(Sender: TObject;
...
procedure TSarongFrm.LookSatuanEnter(Sender: TObject);
begin
  DMFrm.QSatuan.Open;
end;

procedure TSarongFrm.cbOtomatisClick(Sender: TObject);
begin
...
procedure TSarongFrm.ECariChange(Sender: TObject);
begin
  if cbOtomatis.Checked then
  begin
...
procedure TSarongFrm.LTitleClick(Sender: TObject);
var
  DC : HDC;
  ABitmap:TBitmap;
begin
...
procedure TSarongFrm.QMasterBeforePost(DataSet: TDataSet);
begin
// QMasterOPR_EDIT.AsString:=DMFrm.QUserNAMA_USER.AsString;
end;

procedure TSarongFrm.BitBtn6Click(Sender: TObject);
begin
{azmi}
...
procedure TSarongFrm.BitBtn4Click(Sender: TObject);
begin
{azmi}
if QLookUpKelKD_KEL.AsString='' then
begin
...
procedure TSarongFrm.wwDBLookupCombo1CloseUp(Sender: TObject; LookupTable,
  FillTable: TDataSet; modified: Boolean);
begin
{azmi}
...
procedure TSarongFrm.wwDBLookupCombo1Enter(Sender: TObject);
begin
{azmi}
QLookUpKel.Open;
{azmi}
...
procedure TSarongFrm.BitBtn7Click(Sender: TObject);
{azmi}
var
  vuser, vpwd : string;
begin
